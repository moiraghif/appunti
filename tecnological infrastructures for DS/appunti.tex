...
\begin{itemize}[wide, noitemsep, topsep=0ex]
	...
	\item \textbf{CMD} ha lo stesso significato della CMD, ma ha una differenza, se esiste un ENTRYPOINT viene eseguito ENTRYPOINT e CMD gli passa i suoi parametri e se esistono molteplici CMD viene eseguito l'ultimo CMD. Es:
	\begin{verbatim}
		CMD[``--port 27017'']
...
\paragraph{Stream Processing} lavora su un flusso continuo (infinito) di dati, il risultato è un nuovo stream di dati e per ciò è dovuto al fatto che il processo stream non finisce. La funzione in questo casa è applicata ad un datapoint o una finestra piccola di dati recenti, inoltre la computazione deve essere leggera poiché lo stream deve continuare ad andare e la latenza deve essere near real time o al più secondi.\\
...
\begin{myenumerate}
	...
	\item Data Pipeling: è una sequenza di trasformazioni, rappresentate di solito da un grafo aciclico diretto;
...
I Dstream richiedono una buona conoscenza di programmazione, quindi si è creata una sua astrazione sotto il nome id Spark Structured Streaming, che ha delle API a livello più alto rispetto a Dstreams.
