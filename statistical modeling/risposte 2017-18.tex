...
Il \textbf{modello Multilevel} è un modello di analisi della \textbf{covarianza} a effetti casuali: in prima istanza la \textbf{regressione lineare} cattura la relazione disaggregata tra i dati e quindi descrive la \textbf{varianza nei gruppi} mentre, successivamente, \textbf{l’analisi della varianza} cattura la relazione aggregata fra i gruppi e quindi descrive la \textbf{varianza fra gruppi}. Questo modello spezza in due l’analisi mettendo insieme la covarianza:
\begin{enumerate}
...
\begin{enumerate}
\item Per $u_j=0$ e nessuna gerarchia dei dati, \textbf{Modello Lineare}: 
\begin{equation*}
y_i= \gamma_{00}+\Sigma_k \beta_k(x_{ik}-\bar{x}_k) + \varepsilon_i \ ;
\end{equation*}
\item Per $u_j=0$ \textbf{Regressione Multilevel}: 
\begin{equation*}
y_{ij} = \gamma_{00} + \Sigma_k \beta_k(x_{ijk}-\bar{x}_k) + \varepsilon_{ij};
\end{equation*}
\item Per $\Sigma_k \beta_k (x_{ik} - \bar{x}_k) = 0$ e $u_j$ fisso \textbf{Analisi Varianza}: 
\begin{equation*}
y_{ij} = \gamma_{00}+ u_j + \varepsilon_{ij};
\end{equation*}
\item Per $\Sigma_k\beta_k(x_{ik}- \bar{x}_k) = 0$ e $u_j$ stocastico \textbf{Analisi Varianza Casuale}: 
\begin{equation*}
y_{ij} = \gamma_{00} + u_j + \varepsilon_{ij}.
\end{equation*}
\end{enumerate}
