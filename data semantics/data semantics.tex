...
La relazione tra classi e sottoclassi è, matematicamente parlando, un ordinamento parziale: si può costruire un reticolo finito (quindi con limite superiore e inferiore).
Una relazione in OWL ha le proprietà di simmetria e di transitività; inoltre matematicametne è una funzione (ovvero si ha al più un valore per un argomento), permettendo l'uso di identificativi e di funzioni inverse.
Quest'ultima proprietà è fondamentale nelle fasi di \textit{Record Linkage} o \textit{Deduplication}.

Un concetto si dice \textit{soddisfacibile} se esiste almeno una sua interpretazione; invece un concetto si dice \textit{sussunto} a un altro modello se in qualsiasi interpretazione della prima teoria si ha un sottoinsieme della seconda teoria.
...
Due concetti si dicono \textit{equivalenti} se l'interpretazione del primo concetto è sempre uguale a quella del secondo in tutte le interpretazioni della teoria; al contrario si dicono \textit{disgiunti}.
...
Una logica è composta da una TBox, ovvero l'ontologia della logica, e una ABox (\textit{Assertional Box}), che comprende i dati veri e propri.
...
Gli assiomi usati in queste logiche sono estremamente semplici ma si applicano ugualmente anche a classi più complesse.
...
\subsection{RDF4J.}
È un framework per Java per gestire, archiviare (in diversi modi) e interrogare documenti RDF, organizzando tramite un paradigma ad oggetti.

\subsection{Apache Jena.}
È un'api per RDF, offre un reasoner organizzando i dati ad oggetti.

\subsection{\textit{Triplestore}.}
Un \textit{triplestore} archivia le triple e le filtra per elementi fissati.
